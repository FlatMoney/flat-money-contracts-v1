...
contract FlatcoinVault is IFlatcoinVault, OwnableUpgradeable, FeeManager {
    ...
    GlobalPositions internal _globalPositions;

    /// @dev Holds mapping between user addresses and their leverage positions.
    mapping(uint256 tokenId => Position userPosition) internal _positions;
    ...
    function setPosition(Position calldata newPosition_, uint256 tokenId_) external onlyAuthorizedModule {
        if (maxPositions > 0) {
    ...
    function getPosition(uint256 tokenId_) external view returns (Position memory positionDetails_) {
    ...
    function getGlobalPositions() external view returns (GlobalPositions memory globalPositionsDetails_) {
    ...
    function addAuthorizedModules(AuthorizedModule[] calldata modules_) external onlyOwner {
    ...
    function addAuthorizedModule(AuthorizedModule calldata module_) public onlyOwner {
