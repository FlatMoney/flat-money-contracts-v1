...
contract LeverageModule is ILeverageModule, ModuleUpgradeable, ERC721EnumerableUpgradeable {
    ...
    /// @param order_ The order to be executed.
    function executeAdjust(Order calldata order_) external onlyAuthorizedModule {
        IOracleModule oracleModule = IOracleModule(vault.moduleAddress(FlatcoinModuleKeys._ORACLE_MODULE_KEY));
        uint32 maxAge = _getMaxAge(order_.executableAtTime);

        AnnouncedLeverageAdjust memory announcedAdjust = abi.decode(order_.orderData, (AnnouncedLeverageAdjust));

        Position memory position = vault.getPosition(announcedAdjust.tokenId);
    ...
    function executeClose(
        ...
        IOracleModule oracleModule = IOracleModule(vault.moduleAddress(FlatcoinModuleKeys._ORACLE_MODULE_KEY));
        AnnouncedLeverageClose memory announcedClose = abi.decode(order_.orderData, (AnnouncedLeverageClose));

        Position memory position = vault.getPosition(announcedClose.tokenId);
        ...
        PositionSummary memory positionSummary;
        {
    ...
    function getPositionSummary(uint256 tokenId_) external view returns (PositionSummary memory positionSummary_) {
    ...
    function getPositionSummary(
        ...
        uint256 price_
    ) public view returns (PositionSummary memory positionSummary_) {
    ...
    function _update(address to_, uint256 tokenId_, address auth_) internal virtual override returns (address from) {
        ...
        if (tokenOwner != address(0)) {
            Order memory normalOrder = orderAnnouncementModule.getAnnouncedOrder(tokenOwner);
            Order memory limitOrder = orderAnnouncementModule.getLimitOrder(tokenId_);
            ...
            if (normalOrder.orderType == OrderType.LeverageAdjust || normalOrder.orderType == OrderType.LeverageClose) {
                // Decode the order data to check if the order belongs to `tokenId_`.
                if (normalOrder.orderType == OrderType.LeverageAdjust) {
                    AnnouncedLeverageAdjust memory leverageAdjust = abi.decode(
                    ...
                    AnnouncedLeverageClose memory leverageClose = abi.decode(
                        normalOrder.orderData,
            ...
            if (limitOrder.orderType != OrderType.None) {
