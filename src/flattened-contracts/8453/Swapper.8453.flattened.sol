...
abstract contract RouterProcessor is RouterProcessorStorage {
    ...
    function _approveAndCallRouter(
        ...
        if (srcTokenSwapDetails_.token.allowance(address(this), contractToApprove_) < srcTokenSwapDetails_.amount)
            srcTokenSwapDetails_.token.safeIncreaseAllowance(contractToApprove_, type(uint256).max);
