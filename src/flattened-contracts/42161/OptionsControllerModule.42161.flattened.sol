...
abstract contract ControllerBase is IControllerModule, ModuleUpgradeable {
    ...
    function profitLossTotal(uint256 price_) public view virtual returns (int256 pnl_) {
        GlobalPositions memory globalPosition = vault.getGlobalPositions();
    ...
    function accruedFunding(Position memory position_) public view virtual returns (int256 accruedFunding_) {
    ...
    /// @param price_ The current price of the market asset.
    /// @return pnl_ The PnL in terms of the collateral asset.
    function profitLoss(Position memory position_, uint256 price_) public view virtual returns (int256 pnl_) {
...
contract OptionsControllerModule is ControllerBase {
    ...
    function profitLoss(Position memory position_, uint256 price_) public pure override returns (int256 pnl_) {
    ...
    function profitLossTotal(uint256 price_) public view override returns (int256 pnl_) {
        ...
        for (uint256 i; i < _openPositionIds.length; ++i) {
            Position memory position = vault.getPosition(_openPositionIds[i]);
