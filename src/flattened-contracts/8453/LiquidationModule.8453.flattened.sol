    ...
    InvariantChecks
{
    ...
    /// @param priceUpdateData The price update data to be used for updating the Pyth price.
    function liquidate(
        uint256 tokenID,
        bytes[] calldata priceUpdateData
    ...
    function liquidate(uint256 tokenID) public {
        ...
        if (tokenIdArr.length != 1) revert FlatcoinErrors.CannotLiquidate(tokenID);
    }

    /// @notice Function to liquidate multiple positions after Pyth price update.
    /// @param tokenID The token ID of the leverage position.
    /// @param priceUpdateData The price update data to be used for updating the Pyth price.
    /// @return liquidatedIds The token IDs of the liquidated positions.
    function liquidate(
        uint256[] calldata tokenID,
        bytes[] calldata priceUpdateData
    ) external payable updatePythPrice(vault, msg.sender, priceUpdateData) returns (uint256[] memory liquidatedIds) {
        return liquidate(tokenID);
    }
    ...
    function liquidate(
    ...
    ) public nonReentrant whenNotPaused returns (uint256[] memory liquidatedIds) {
        ...
        liquidatedIds = new uint256[](tokenID.length);
        uint256 counter;

        for (uint256 i; i < tokenID.length; ++i) {
            if (canLiquidate(tokenID[i], currentPrice)) {
                _processLiquidation(tokenID[i], currentPrice);

                liquidatedIds[counter] = tokenID[i];
                ++counter;
            }
        }

        uint256 reduceLength = tokenID.length - counter;

        // solhint-disable-next-line no-inline-assembly
        assembly {
            mstore(liquidatedIds, sub(mload(liquidatedIds), reduceLength))
        }
    }
    ...
    function _processLiquidation(
        uint256 tokenId,
        uint256 currentPrice
    ) private liquidationInvariantChecks(vault, tokenId) {
        FlatcoinStructs.Position memory position = vault.getPosition(tokenId);
        ...
        ILeverageModule(vault.moduleAddress(FlatcoinModuleKeys._LEVERAGE_MODULE_KEY)).burn(
            tokenId,
