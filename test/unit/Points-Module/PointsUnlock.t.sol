...
contract PointsUnlockTest is Setup, OrderHelpers, ExpectRevert {
    ...
    function test_points_unlock_vested_after_remint() public {
        vm.startPrank(admin);

        PointsModule.MintPoints memory mintParams = PointsModule.MintPoints({to: alice, amount: 1000e18});
        pointsModProxy.mintTo(mintParams);

        skip(365 days + 1); // 100% vest

        vm.startPrank(admin);

        mintParams = PointsModule.MintPoints({to: alice, amount: 1e18});
        pointsModProxy.mintTo(mintParams);

        vm.startPrank(alice);

        assertEq(pointsModProxy.lockedBalance(alice), 1e18); // only the newly minted point should be locked
        assertEq(pointsModProxy.unlockTime(alice), block.timestamp + 365 days); // the newly minted point should be locked for 1 year

        pointsModProxy.unlock(type(uint256).max);

        assertEq(pointsModProxy.balanceOf(alice), 1000e18);
        assertEq(pointsModProxy.balanceOf(treasury), 1e18);
        assertEq(pointsModProxy.lockedBalance(alice), 0e18);
    }
