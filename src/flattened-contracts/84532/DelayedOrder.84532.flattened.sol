    ...
    OracleModifiers
{
    ...
    function announceStableDeposit(uint256 depositAmount, uint256 minAmountOut, uint256 keeperFee) external {
        announceStableDepositFor(depositAmount, minAmountOut, keeperFee, msg.sender);
    }
    ...
    function announceLeverageOpen(
        ...
        announceLeverageOpenFor(margin, additionalSize, maxFillPrice, keeperFee, msg.sender);
    }
    ...
    /// @param receiver The receiver address of the token received back
    function announceStableDepositFor(
        uint256 depositAmount,
        uint256 minAmountOut,
        uint256 keeperFee,
        ...
        uint64 executableAtTime = _prepareAnnouncementOrder(keeperFee, receiver);
        ...
        _announcedOrder[receiver] = FlatcoinStructs.Order({
    ...
    function announceStableWithdraw(
        ...
        uint64 executableAtTime = _prepareAnnouncementOrder(keeperFee, msg.sender);
        ...
        orderInvariantChecks(vault)
    {
        ...
        if (orderType == FlatcoinStructs.OrderType.None) revert FlatcoinErrors.DelayedOrderInvalid(account);
    ...
    /// @dev Uses the Pyth network price to execute.
    /// @param account The user account which has a pending deposit.
    function _executeStableDeposit(address account) internal {
        FlatcoinStructs.Order memory order = _announcedOrder[account];
    ...
    function _executeStableWithdraw(address account) internal {
        FlatcoinStructs.Order memory order = _announcedOrder[account];
    ...
    /// @dev Uses the Pyth network price to execute.
    /// @param account The user account which has a pending order.
    function _executeLeverageOpen(address account) internal {
        FlatcoinStructs.Order memory order = _announcedOrder[account];
        ...
        ILeverageModule(vault.moduleAddress(FlatcoinModuleKeys._LEVERAGE_MODULE_KEY)).executeOpen({
            account: account,
            keeper: msg.sender,
            order: order
        });
    ...
    /// @dev Uses the Pyth network price to execute.
    /// @param account The user account which has a pending order.
    function _executeLeverageClose(address account) internal {
        FlatcoinStructs.Order memory order = _announcedOrder[account];
        FlatcoinStructs.AnnouncedLeverageClose memory leverageClose = abi.decode(
            order.orderData,
            (FlatcoinStructs.AnnouncedLeverageClose)
    ...
    function _prepareAnnouncementOrder(uint256 keeperFee, address receiver) internal returns (uint64 executableAtTime) {
