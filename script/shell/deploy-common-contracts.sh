...
# Function signature of the function "deployCommonContracts(string[])"
FUNC_SIG="0xadcdfabd"

# Get abi-encoded parameters for the function.
# Don't proceed if there is an error after calling cast.
ENCODED_PARAMS=$(cast calldata "deployCommonContracts(string[])" "$MODULE_NAMES_FORMATTED" || exit 1)
...
if [[ $MODE == "dry" ]]; then
    ...
    pnpm run:oz script/tasks/deploy-common-contract.s.sol \
    --sig $ENCODED_PARAMS\
    --rpc-url $CHAIN_NAME \
    --account $ACCOUNT_KEY \
